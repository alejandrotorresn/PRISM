...
def plot_profiling_layer_costs() -> None:
    ...
    ax.bar([p + width for p in positions], transfer, width=width, label="Edge-aware transfer (ms)", color="#2F855A")

    ax.set_xticks(positions)
    ax.set_xticklabels(layers)
    ax.set_ylabel("Milliseconds")
    ax.set_title("Per-layer profiling: simple_mlp, batch 8, fp32")
