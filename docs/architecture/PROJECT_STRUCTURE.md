...
├── docs/         # Indice documental y subarboles architecture/, operations/, thesis/, archive/
├── logs/         # Trazas por workflow y archivo historico
├── reports/      # Resultados ILP, figuras de tesis y diagnosticos separados
...
`scripts/` concentra la orquestacion y analisis. `run_experiments.sh` gobierna campañas de profiling por grilla; `run_thesis_mode.sh` y `run_thesis.sh` coordinan el flujo doctoral extremo a extremo (incluyendo lanzamientos en Grid5000); `run_thesis_smoke_workflow.sh` permite una ejecucion reducida de comprobacion. Para optimizacion ILP, `run_ilp_partition.sh` y `run_ilp_pareto_sweep.sh` envuelven la fase de resolucion. Para analisis multi-nodo, `find_free_gpus.py` busca recursos en Grid5000, `discover_ilp_config_dirs.sh` facilita la ejecucion multi-host, y `generate_multiserver_summary.py` analiza el consolidado global. En validacion y auditoria de la tesis, operan `audit_experimental_grid.sh` y `run_statistical_significance.sh`. `download_datasets.py` prepara los datasets persistentes requeridos. En el eje de reporte, `generate_ilp_report_assets.sh` y `export_ilp_tables_latex.sh` construyen tablas; mientras que `generate_thesis_figures.py`, `generate_methodological_plots.py`, `plot_all_energy_tradeoff.py`, `plot_all_oom_vs_ilp.py`, `organize_plots.py` y (desde la raiz) `plot_custom_user_figures.py` manejan la extraccion de evidencia grafica, formalmente testeada y mantenida. Finalmente, `launch_grid5k.sh` atiende escenarios HPC legados, y `sanitize_cuda_env.sh` reduce fragilidad ambiental. El subdirectorio `scripts/archive/` aisla scripts temporales y parches obsoletos de datos (dirty hacks) para no contaminar la orquestacion oficial.
...
Para una incorporacion rapida al proyecto conviene comenzar por [README.md](../../README.md), continuar con [GLOBAL_PROJECT_DOCUMENTATION_ES.md](GLOBAL_PROJECT_DOCUMENTATION_ES.md) como referencia tecnica canonica en espanol, contrastar con [GLOBAL_PROJECT_DOCUMENTATION.md](GLOBAL_PROJECT_DOCUMENTATION.md) cuando se necesite la version inglesa, y despues entrar en [PROTOCOLO_VALIDACION_MULTISERVIDOR_ES.md](../operations/PROTOCOLO_VALIDACION_MULTISERVIDOR_ES.md), [SERVER_LAUNCH_PROFILES.md](../operations/SERVER_LAUNCH_PROFILES.md) y [MULTI_NODE_ILP_RUNBOOK.md](../operations/MULTI_NODE_ILP_RUNBOOK.md) para la operacion real sobre servidores y escenarios multi-host. Este documento debe leerse como mapa estructural; la semantica detallada de CLI, formulas y protocolos vive en la documentacion global y en los runbooks especializados.
