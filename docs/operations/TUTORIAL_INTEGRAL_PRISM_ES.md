...
- `docs/architecture/PROJECT_STRUCTURE.md`: cartografía estructural del repositorio y responsabilidades por bloque.
- `docs/architecture/GLOBAL_PROJECT_DOCUMENTATION_ES.md`: referencia técnica integral en español.
- `docs/architecture/GLOBAL_PROJECT_DOCUMENTATION.md`: referencia técnica integral en inglés.
- `docs/thesis/CAPITULO_TESIS_PROFILING_ES.md`: desarrollo monográfico de la metodología de profiling.
- `docs/thesis/CAPITULO_TESIS_ILP_ES.md`: desarrollo monográfico de la formulación ILP y su validación.
- `docs/operations/PROTOCOLO_VALIDACION_MULTISERVIDOR_ES.md`: protocolo maestro para campañas reales multiservidor.
- `docs/operations/SERVER_LAUNCH_PROFILES.md`: perfiles de ejecución recomendados por clase de servidor.
- `docs/operations/MULTI_NODE_ILP_RUNBOOK.md`: guía operativa para fusión multi-host y resolución ILP sobre hardware heterogéneo.
- `docs/thesis/schema.md`: mapa de escritura de la monografía doctoral.
- `docs/operations/QUICK_START.sh`: ayuda rápida en shell para comandos frecuentes del proyecto.
- `docs/archive/reference_material/A New Parallelization Approach in Deep Learning Using CPU.docx`: documento ofimático heredado asociado al trabajo doctoral.
...
`logs/` contiene bitácoras cronológicas de ejecución. No reemplaza a los CSV de resultados; su función es proveer trazabilidad operacional, mensajes de error, secuencia de pasos y tiempo de cada campaña. En la estructura actual, cada workflow escribe en su propio subdirectorio para evitar saturación visual del nivel superior.

Los patrones visibles son:

- `logs/experiments/experiments_YYYYMMDD_HHMMSS.txt`: logs de `scripts/run_experiments.sh`.
- `logs/thesis_mode/thesis_mode_YYYYMMDD_HHMMSS.txt`: logs de `scripts/run_thesis_mode.sh`.
- `logs/thesis_smoke_workflow/thesis_smoke_workflow_YYYYMMDD_HHMMSS.txt`: logs de `scripts/run_thesis_smoke_workflow.sh`.
- `logs/grid5k/grid5k_launch_YYYYMMDD_HHMMSS.log`: logs de `scripts/launch_grid5k.sh`.
...
Flujo diagnóstico doctoral (fallbacks explícitos para depuración):

```bash
PROFILE=doctoral_diagnostic bash scripts/run_thesis_mode.sh
```
...
1. leer `README.md`
2. leer `docs/architecture/PROJECT_STRUCTURE.md`
