...
- quick_smoke, doctoral_minimal y doctoral_diagnostic: semilla unica (verificacion operativa)
...
- Si `--profile doctoral_full`:
  ...
  - repeticion por semilla controlada por `--full-repeats` (default 2)

- Si `--profile` es distinto de doctoral_full (`quick_smoke`, `doctoral_minimal` o `doctoral_diagnostic`):
  - ejecuta una sola semilla `--single-seed` (default 42)
  - `doctoral_minimal` usa al menos 2 réplicas; `quick_smoke` y `doctoral_diagnostic` pueden usar 1
...
En modo estricto, el launcher eleva automaticamente esta configuracion a 2 replicas.

### 4.2.b Diagnostico doctoral controlado (doctoral_diagnostic)
Comando:
...
- `--single-seed`: semilla unica para quick_smoke/doctoral_minimal/doctoral_diagnostic
- `--non-full-repeats`: replicas en quick_smoke/doctoral_minimal/doctoral_diagnostic
...
- Logs de launcher: carpeta `logs/grid5k/` del proyecto remoto
...
4. Si faltan artefactos: revisar logs en `logs/grid5k/` y `logs/thesis_mode/` y confirmar que no se uso `DRY_RUN=true`.

## 9. Flujo operativo sugerido para campana final
1. Ejecutar quick_smoke (1 semilla).
2. Ejecutar doctoral_minimal (1 semilla y al menos 2 replicas).
3. Ejecutar doctoral_diagnostic solo si hay necesidad de depuracion instrumental.
4. Ejecutar doctoral_full con 3-5 semillas y al menos 2 replicas por semilla.
5. Consolidar resultados y generar reportes finales.
